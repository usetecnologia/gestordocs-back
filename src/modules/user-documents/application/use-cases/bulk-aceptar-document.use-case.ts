...
export interface BulkReviewDocumentErrorItem {
  dni: string;
  reason: string;
}

export interface BulkReviewDocumentResult {
  totalSuccess: number;
  totalErrors: number;
  successes: string[];
  errors: BulkReviewDocumentErrorItem[];
}

@Injectable()
export class BulkAceptarDocumentUseCase {
  constructor(
  ...
  async execute(
    ...
    for (let i = 0; i < dnis.length; i += CONCURRENCY) {
      ...
      const results = await Promise.allSettled(
        batch.map((dni) => this.processDni(dni, documentId, sponsorId ?? null, reviewedById)),
      );
  ...
  private async processDni(
    ...
    await this.aceptarDocumentUseCase.execute(userDocumentId, reviewedById);
    return userId;
  }
