...
async function main() {
  ...
  // DOCUMENTO_OBSERVADO (ver TerminarRevisionUseCase: dispara un único correo por revisión,
  // no uno por cada documento observado) y USER_OBSERVADO (POST /users/observations: dispara
  // un correo cada vez que se registra una observación directa sobre el participante).
  const emailActions: Array<{ name: string; code: string }> = [
    { name: 'Documento con estado observado', code: 'DOCUMENTO_OBSERVADO' },
    { name: 'Participante observado', code: 'USER_OBSERVADO' },
  ];

  for (const action of emailActions) {
    await prisma.emailAction.upsert({
      where: { code: action.code },
      update: { name: action.name, status: true },
      create: { name: action.name, code: action.code, status: true },
    });
  }
